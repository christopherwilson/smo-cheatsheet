\subsubsection*{Transience \& Recurrence}
\wde{Prob. of Arrival} Define $T_j = \min\{n \ge 1 : X_n = j\}$ to be the first arrival time to state $j$, and $\varrho_{ij} = P(T_j < \infty|X_0 = i) = \sum^\infty_{n=1}P(T_j = n| X_0 = i)$ be the probability of ever reaching $j$ from $i$.
\wde{Recurrent \& Transient 2.6.1} A state $i$ is said to be recurrent if $\varrho_{ii} = 1$ and it said to be transient if $\varrho_{ii} < 1$.
...
\wc{2.6.4} (a) if $i \to j$ and $i$ is recurrent then $\varrho_{ji} = 1$; 
...
\wl{Ratio Test 2.6.9} $\sum^{\infty}_{k=0} a_k$ converges if $\lim_{k\to \infty} a_{k+1}/a_k < 1$, and it diverges if $\lim_{k\to \infty} a_{k+1}/a_k > 1$. If $\lim_{k\to \infty} a_{k+1}/a_k = 1$ the test says nothing.
